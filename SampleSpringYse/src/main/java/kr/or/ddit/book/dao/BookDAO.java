...
public class BookDAO {
	 ...
	 * 
	 * 외부에서 Dao까지 map에 title, categroy, price가 담겨져서 넘어온다.
	...
	public Map<String, Object> selectBook(Map<String, Object> map) {
		 ...
		 * 만약 퀴리 결과 행 수가 0개면 selectOne 메소드는 null을 반환하게 되고,
		 * 쿼리 결과가 여러 개면 ToomanyResultException 예외를 던진다.
		 * 우리가 작성한 쿼리는 조건이 pk이고, pk는 무조건 행이 유일함을 보장하기 때문에 결과를 0개 아니면 1개이다.
		 * commit은 알아서 템플릿 상으로 해준다.
		 */
	...
	public int updateBook(Map<String, Object> map) {	
		 ...
		 * 첫번째 파라미터는 쿼리 ID, 두번째 파라미터는 쿼리 파라미터이고 반환값을 영향받은 행 수이다.
		 */
	...
	public int removeBook(Map<String, Object> map) {
		/*
		 * sqlSessionTemplate객체의 deleteBook 메소드는 update 메소드와 사용법이 동일하다
		 * 첫번째 파라미터는 쿼리 ID, 두번째 파라미터는 쿼리 파라미터이고 반환값은 영향받은 행 수이다.
		 */
